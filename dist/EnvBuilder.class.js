...
const fs = require('fs');
class EnvBuilder {
    static async GenerateTemplate(tplName = 'ENV.list.ts') {
    ...
    generate(settings) {
        ...
        function addLine(env, notRequired = false, compact = false) {
            const mutePrefix = notRequired ? '#' : '';
            if (typeof env.defaultValue !== "string")
                env.defaultValue = JSON.stringify(env.defaultValue);
            if (env.description && env.description.length && !settings.noComments)
                fileString += `# ↓ ${env.description}\n`;
    ...
    _checkForRequiredEnvs(fileName) {
        const allEnvs = this._getEnvsFromFile(fileName);
        if (!allEnvs)
            throw `Failed to load data from "${fileName}"`;
        let isErr = false;
...
//# sourceMappingURL=EnvBuilder.class.js.map